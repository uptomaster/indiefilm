"use client";

import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { useAuth } from "@/hooks/useAuth";
import {
  getFilmmakerByUserId,
  getFilmmakerMovies,
  Filmmaker,
} from "@/lib/filmmakers";
import { Movie } from "@/lib/movies";
import { getUnreadRequestCount } from "@/lib/requests";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";

export default function FilmmakerProfileViewPage() {
  const router = useRouter();
  const { user, userProfile, loading: authLoading } = useAuth();
  const [filmmaker, setFilmmaker] = useState<Filmmaker | null>(null);
  const [movies, setMovies] = useState<Movie[]>([]);
  const [loading, setLoading] = useState(true);
  const [unreadCount, setUnreadCount] = useState(0);

  useEffect(() => {
    if (authLoading) {
      return;
    }
    if (!user) {
      router.push("/login");
      return;
    }
    if (userProfile) {
      if (!userProfile.role || userProfile.role !== "filmmaker") {
        router.push("/role-select");
        return;
      }
      loadProfile();
      loadUnreadCount();
    }
  }, [user, userProfile, authLoading]);

  const loadUnreadCount = async () => {
    if (!user) return;
    try {
      const count = await getUnreadRequestCount(user.uid);
      setUnreadCount(count);
    } catch (error) {
      console.error("Error loading unread count:", error);
    }
  };

  const loadProfile = async () => {
    if (!user) return;

    try {
      setLoading(true);
      const filmmakerData = await getFilmmakerByUserId(user.uid);
      if (!filmmakerData) {
        router.push("/filmmakers/me/edit");
        return;
      }
      setFilmmaker(filmmakerData);

      // 제작한 영화 목록 로드
      const filmmakerMovies = await getFilmmakerMovies(filmmakerData.id);
      setMovies(filmmakerMovies);
    } catch (error) {
      console.error("Error loading profile:", error);
    } finally {
      setLoading(false);
    }
  };

  if (authLoading || loading) {
    return (
      <div className="min-h-screen bg-black text-[#faf6f0]">
        <div className="container mx-auto px-4 py-20">
          <div className="text-center">
            <div className="inline-block h-12 w-12 animate-spin rounded-full border-4 border-[#e8a020] border-t-transparent" />
            <p className="mt-4 text-[#b8a898]">로딩 중...</p>
          </div>
        </div>
      </div>
    );
  }

  if (!filmmaker) {
    return null;
  }

  return (
    <div className="min-h-screen bg-white text-[#faf6f0]">
      {/* 히어로 섹션 - 배경만 */}
      <div className="relative h-[40vh] min-h-[300px] overflow-hidden border-b border-[#e8a020]/30 bg-gradient-to-b from-[#100e0a] to-[#0a0805]">
        <div className="film-strip absolute inset-0 opacity-10" />
        
        {/* 상단 네비게이션 */}
        <div className="absolute left-0 right-0 top-0 z-20">
          <div className="container mx-auto flex items-center justify-between px-4 py-3 md:py-6 gap-2">
            <Link
              href="/filmmakers"
              className="rounded-full bg-white/90 px-3 md:px-4 py-1.5 md:py-2 text-xs md:text-sm text-[#faf6f0] backdrop-blur-sm transition-colors hover:bg-[#100e0a] border border-[#e8a020]/30 shadow-sm font-semibold whitespace-nowrap"
            >
              ← 제작자 목록
            </Link>
            <div className="flex items-center gap-2">
              <Link href="/requests">
                <Button
                  variant="outline"
                  className="relative border-[#e8a020] text-[#e8a020] hover:bg-[#e8a020]/10 text-xs md:text-sm px-2 md:px-3 py-1 md:py-1.5"
                >
                  요청 확인
                  {unreadCount > 0 && (
                    <span className="absolute -right-1 -top-1 flex h-4 w-4 md:h-5 md:w-5 items-center justify-center rounded-full bg-[#e8a020] text-[10px] md:text-xs font-bold text-white">
                      {unreadCount > 9 ? "9+" : unreadCount}
                    </span>
                  )}
                </Button>
              </Link>
              <Button
                onClick={() => router.push("/filmmakers/me/edit")}
                className="btn-primary-gradient text-white font-semibold text-xs md:text-sm px-3 md:px-4 py-1.5 md:py-2 whitespace-nowrap"
              >
                프로필 수정
              </Button>
            </div>
          </div>
        </div>
      </div>

      {/* 메인 콘텐츠 */}
      <div className="container mx-auto px-4 py-6 md:py-12">
        <div className="grid gap-6 md:gap-12 lg:grid-cols-3">
          {/* 좌측: 메인 정보 */}
          <div className="lg:col-span-2 space-y-8">
            {/* 프로필 헤더 */}
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6 pb-8 border-b border-[#e8a020]/30">
              {/* 프로필 사진 */}
              <div className="flex-shrink-0">
                <div className="relative h-32 w-32 md:h-40 md:w-40 rounded-full overflow-hidden border-4 border-[#e8a020]/30 shadow-2xl">
                  {filmmaker.mainPhotoUrl ? (
                    <img
                      src={filmmaker.mainPhotoUrl}
                      alt={filmmaker.name}
                      className="h-full w-full object-cover"
                    />
                  ) : (
                    <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-[#0d0b08] to-[#0a0805]">
                      <span className="text-4xl md:text-5xl">🎬</span>
                    </div>
                  )}
                </div>
              </div>
              
              {/* 기본 정보 */}
              <div className="flex-1">
                <div className="mb-2">
                  <span className="rounded-full bg-[#e8a020]/20 px-3 py-1 text-sm text-[#e8a020]">
                    {filmmaker.type === "team" ? "팀" : "개인"}
                  </span>
                </div>
                <h1 className="mb-4 text-4xl md:text-5xl font-bold film-gold">
                  {filmmaker.name}
                </h1>
                <div className="flex flex-wrap gap-4 text-lg">
                  {filmmaker.location && (
                    <span className="text-[#b8a898]">{filmmaker.location}</span>
                  )}
                  {filmmaker.website && (
                    <>
                      <span className="text-[#b8a898]">•</span>
                      <a
                        href={filmmaker.website}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-[#e8a020] hover:text-[#e8a020]/80"
                      >
                        웹사이트
                      </a>
                    </>
                  )}
                </div>
              </div>
            </div>
            {/* 소개 */}
            {filmmaker.bio && (
              <section>
                <h2 className="mb-4 border-b border-[#e8a020]/30 pb-2 text-2xl font-bold film-gold">
                  PROFILE
                </h2>
                <p className="whitespace-pre-wrap text-sm md:text-base lg:text-lg leading-relaxed text-[#b8a898]">
                  {filmmaker.bio}
                </p>
              </section>
            )}

            {/* 전문 분야 */}
            {filmmaker.specialties && filmmaker.specialties.length > 0 && (
              <section>
                <h2 className="mb-4 border-b border-[#e8a020]/30 pb-2 text-2xl font-bold film-gold">
                  SPECIALTIES
                </h2>
                <div className="flex flex-wrap gap-2">
                  {filmmaker.specialties.map((specialty, index) => (
                    <span
                      key={index}
                      className="rounded-full bg-[#e8a020]/20 px-4 py-2 text-[#e8a020]"
                    >
                      {specialty}
                    </span>
                  ))}
                </div>
              </section>
            )}

            {/* 경력 */}
            {filmmaker.experience && filmmaker.experience.length > 0 && (
              <section>
                <h2 className="mb-4 border-b border-[#e8a020]/30 pb-2 text-2xl font-bold film-gold">
                  EXPERIENCE
                </h2>
                <ul className="space-y-3">
                  {filmmaker.experience.map((exp, index) => (
                    <li
                      key={index}
                      className="flex items-start gap-3 border-l-2 border-[#e8a020]/30 pl-4"
                    >
                      <span className="text-[#e8a020]">▸</span>
                      <span className="text-[#b8a898]">{exp}</span>
                    </li>
                  ))}
                </ul>
              </section>
            )}

            {/* 보유 장비 */}
            {filmmaker.equipment && filmmaker.equipment.length > 0 && (
              <section>
                <h2 className="mb-4 border-b border-[#e8a020]/30 pb-2 text-2xl font-bold film-gold">
                  EQUIPMENT
                </h2>
                <div className="flex flex-wrap gap-2">
                  {filmmaker.equipment.map((item, index) => (
                    <span
                      key={index}
                      className="rounded-full bg-[#0d0b08]/80 px-4 py-2 text-[#b8a898]"
                    >
                      {item}
                    </span>
                  ))}
                </div>
              </section>
            )}

            {/* 팀 멤버 */}
            {filmmaker.type === "team" &&
              filmmaker.teamMembers &&
              filmmaker.teamMembers.length > 0 && (
                <section>
                  <h2 className="mb-4 border-b border-[#e8a020]/30 pb-2 text-2xl font-bold film-gold">
                    TEAM MEMBERS
                  </h2>
                  <div className="space-y-3">
                    {filmmaker.teamMembers.map((member, index) => (
                      <div
                        key={index}
                        className="flex items-start gap-3 border-l-2 border-[#e8a020]/30 pl-4"
                      >
                        <div>
                          <span className="font-semibold text-[#e8a020]">
                            {member.name}
                          </span>
                          <span className="ml-2 text-[#b8a898]">
                            - {member.role}
                          </span>
                          {member.profileLink && (
                            <a
                              href={member.profileLink}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="ml-2 text-[#e8a020] hover:text-[#e8a020]/80 hover:underline"
                            >
                              프로필 →
                            </a>
                          )}
                        </div>
                      </div>
                    ))}
                  </div>
                </section>
              )}

            {/* 제작 영화 */}
            {movies.length > 0 && (
              <section>
                <h2 className="mb-4 border-b border-[#e8a020]/30 pb-2 text-2xl font-bold film-gold">
                  FILMOGRAPHY
                </h2>
                <div className="grid gap-6 md:grid-cols-2">
                  {movies.map((movie) => (
                    <div key={movie.id} className="relative group">
                      <Link href={`/movies/${movie.id}`}>
                        <Card className="border-[#e8a020]/20 bg-[#100e0a] hover:border-[#e8a020]/50 transition-colors cursor-pointer">
                          <CardContent className="p-0">
                            {movie.thumbnailUrl && (
                              <div className="aspect-video overflow-hidden">
                                <img
                                  src={movie.thumbnailUrl}
                                  alt={movie.title}
                                  className="h-full w-full object-cover"
                                />
                              </div>
                            )}
                            <div className="p-4">
                              <h3 className="text-xl font-bold text-[#faf6f0] mb-2">
                                {movie.title}
                              </h3>
                              <p className="text-sm text-[#b8a898] line-clamp-2">
                                {movie.logline}
                              </p>
                            </div>
                          </CardContent>
                        </Card>
                      </Link>
                      <Link
                        href={`/movies/${movie.id}/edit`}
                        className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        <Button
                          size="sm"
                          variant="outline"
                          className="bg-black/80 backdrop-blur-sm border-[#e8a020]/50 text-[#e8a020] hover:bg-[#e8a020]/20"
                        >
                          수정
                        </Button>
                      </Link>
                    </div>
                  ))}
                </div>
              </section>
            )}

            {/* 갤러리 */}
            {filmmaker.gallery && filmmaker.gallery.length > 0 && (
              <section>
                <h2 className="mb-4 border-b border-[#e8a020]/30 pb-2 text-2xl font-bold film-gold">
                  GALLERY
                </h2>
                <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
                  {filmmaker.gallery.map((item, index) => (
                    <div
                      key={index}
                      className="aspect-square overflow-hidden rounded-lg cinematic-shadow"
                    >
                      <img
                        src={item.url}
                        alt={`Gallery ${index + 1}`}
                        className="h-full w-full object-cover"
                      />
                    </div>
                  ))}
                </div>
              </section>
            )}
          </div>

          {/* 우측: 사이드바 */}
          <div className="lg:col-span-1">
            <div className="sticky top-8 space-y-6">
              {/* 프로필 정보 */}
              <div className="rounded-lg border border-[#e8a020]/20 bg-[#100e0a] p-6 backdrop-blur-sm">
                <h3 className="mb-4 text-sm font-semibold text-[#b8a898]">
                  프로필 정보
                </h3>
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    <span className="text-[#b8a898]">유형</span>
                    <span className="text-[#faf6f0]">
                      {filmmaker.type === "team" ? "팀" : "개인"}
                    </span>
                  </div>
                  {filmmaker.location && (
                    <div className="flex justify-between">
                      <span className="text-[#b8a898]">지역</span>
                      <span className="text-[#faf6f0]">{filmmaker.location}</span>
                    </div>
                  )}
                  {filmmaker.email && (
                    <div className="flex justify-between">
                      <span className="text-[#b8a898]">이메일</span>
                      <a
                        href={`mailto:${filmmaker.email}`}
                        className="text-[#e8a020] hover:text-[#e8a020]/80"
                      >
                        {filmmaker.email}
                      </a>
                    </div>
                  )}
                  {filmmaker.phone && (
                    <div className="flex justify-between">
                      <span className="text-[#b8a898]">전화번호</span>
                      <a
                        href={`tel:${filmmaker.phone}`}
                        className="text-[#e8a020] hover:text-[#e8a020]/80"
                      >
                        {filmmaker.phone}
                      </a>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span className="text-[#b8a898]">공개 상태</span>
                    <span
                      className={
                        filmmaker.isPublic ? "text-green-400" : "text-[#b8a898]"
                      }
                    >
                      {filmmaker.isPublic ? "공개" : "비공개"}
                    </span>
                  </div>
                </div>
              </div>

              {/* 제작 영화 통계 */}
              <div className="rounded-lg border border-[#e8a020]/20 bg-[#100e0a] p-6 backdrop-blur-sm">
                <h3 className="mb-4 text-sm font-semibold text-[#b8a898]">
                  제작 통계
                </h3>
                <div className="text-center">
                  <div className="text-3xl font-bold text-[#e8a020]">
                    {movies.length}
                  </div>
                  <div className="text-sm text-[#b8a898] mt-1">제작 영화</div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
